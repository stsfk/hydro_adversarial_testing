...
# %%
def extract_and_predict_catchment_discharge_and_x_grad_sign(catchment_index, noise=1e-3):
    ...
    pseudo_labels = preds.clone().detach()
    pseudo_labels = pseudo_labels + torch.randn_like(pseudo_labels) * noise

    # compute the pseudo_gradient  
    pseudo_loss = mse_loss_with_nans(preds, pseudo_labels) # the target is 
    pseudo_loss.backward()
    pseudo_loss = pseudo_loss.detach().cpu().numpy()

    grid_sign = x.grad.sign()
    
    # compute real loss
    with torch.no_grad():
        loss = mse_loss_with_nans(preds, y).detach().cpu().numpy()
...
combinations = list(
    itertools.product(catchment_indices, epsilon_values, P_reasonable, repeats)
)

# Create a DataFrame from the combinations
df = pd.DataFrame(combinations, columns=["catchment_index", "epsilon", "P_reasonable", "repeats"])
...
# %%
df.to_csv('data/results/P_reasonable_pseudo_labels.csv', sep=',', index=False)
...
# Generate all unique combinations of input variables
combinations = list(
    itertools.product(catchment_indices, epsilon_values, P_change_when_it_rains, repeats)
)

# Create a DataFrame from the combinations
df = pd.DataFrame(combinations, columns=["catchment_index", "epsilon", "P_change_when_it_rains", "repeats"])
...
# %%
df.to_csv('data/results/P_change_when_it_rains_pseudo_labels.csv', sep=',', index=False)
...
# Generate all unique combinations of input variables
combinations = list(
    itertools.product(catchment_indices, epsilon_values, P_unchange, repeats)
)

# Create a DataFrame from the combinations
df = pd.DataFrame(combinations, columns=["catchment_index", "epsilon", "P_unchange", "repeats"])
...
# %%
df.to_csv('data/results/P_unchange_pseudo_labels.csv', sep=',', index=False)
